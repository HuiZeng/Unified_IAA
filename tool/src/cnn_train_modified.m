...
% -------------------------------------------------------------------------
% predictions = gather(res(end-1).x) ;
res(end-1).x = vl_nnsoftmax(res(end-1).x);
predictions = squeeze(gather(res(end-1).x))' ;
if size(predictions,1) == 1
    error = bsxfun(@times, predictions, labels) < 0 ;
    err = sum(error(:)) ;
else
    predictions = predictions * params.imdb.anchors;
    X = ones(1,numel(labels));
    X(find(predictions<=5)) = -1;
    error = (X .* labels) < 0;
    err = sum(error(:)) ;

%     predictions = predictions(:,1:10) * params.imdb.weights;
%     X = ones(1,numel(labels));
%     X(find(predictions<=0.5)) = -1;
%     error = (X .* labels) < 0;
%     err = sum(error(:)) ;
end
...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    [im, labels, imgID] = params.getBatch(params.imdb, batch) ;
%     net.layers{end}.score_weights = params.imdb.score_weights(batch);
%     net.layers{31}.saliencymap = saliencymap;
    if strcmp(net.layers{end-7}.type,'multiCrop_agg')
        net.layers{end-7}.imgID = imgID;
    end
...
%             idx = find(imgID == i);
%             sub_im = im(:,:,:,idx) ;
%             net.layers{31}.saliencymap = saliencymap(:,:,i);
%             net.layers{end-7}.imgID = imgID(idx);
%             net.layers{end}.class = labels(:,i) ;
%             sub_res = vl_simplenn_modified(net, sub_im, dzdy, [], ...
%                               'accumulate', s ~= 1, ...
%                               'mode', evalMode, ...
%                               'conserveMemory', params.conserveMemory, ...
%                               'backPropDepth', params.backPropDepth, ...
...
%                         res(l).dzdw{j} = res(l).dzdw{j} + sub_res(l).dzdw{j};
%                     end
%                 end
%                 res(end-1).x = cat(4,res(end-1).x,sub_res(end-1).x);
%                 res(end-3).aux = cat(4,res(end-3).aux,sub_res(end-3).aux);
%                 res(end).x = res(end).x + sub_res(end).x;
%             end
%         end
%     end
...
%       reshape(params.errorFunction(params, params.imdb.images.labels(:,batch), res),[],1) ; ]],2) ;
  
    groundtruth = params.imdb.images.labels(:,batch);
    X = ones(1,numel(groundtruth));
    if size(res(end-1).x,3) > 1
        switch params.imdb.meta.lossType
            case 'CE'
                predictions(batch) = params.imdb.anchors' * squeeze(gather(vl_nnsoftmax(res(end-1).x)));
            case {'MSE','Huber'}
                predictions(batch) = params.imdb.anchors' * squeeze(gather(res(end-1).x));
        end
        X(predictions(batch)<=5) = -1;
    else
        switch params.imdb.meta.lossType
            case 'CE'
                predictions(batch) = squeeze(gather(vl_nnsigmoid(res(end-1).x))); 
            case {'MSE','Huber'}
                predictions(batch) = squeeze(gather(res(end-1).x)); 
        end
        X(predictions(batch)<=0.5) = -1;
        if strcmp(params.imdb.database,'AVA')
            predictions(batch) = predictions(batch) * 10;
        end
    end
    error = (X .* groundtruth) < 0;
    err = err + sum(error(:)) ;
    mse = mse + sum((params.imdb.images.score(:,batch)-predictions(batch)).^2);
    mae = mae + sum(abs(params.imdb.images.score(:,batch)-predictions(batch)));
  end
...
stats.srcc = corr(predictions(subset)',params.imdb.images.score(subset)','type','Spearman');
...
% net.layers(6:end) = sub_net2.layers;
