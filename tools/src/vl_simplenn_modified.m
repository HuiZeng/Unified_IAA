function res = vl_simplenn_modified(net, x, dzdy, res, varargin)
...
%% QLW 2016-9-6
ropts.alpha = 0.3; %% Brid 0.3 Aircraft 0.3 Cars 0.5
ropts.epsilon_inter = 0.5;
ropts.epsilon = 1e-3;
ropts.fm = 'hellfm'; %% 'logfm'
ropts.fmp = 0.5; %% Brid Cars Aircraft 0.5
...
for i=1:n
  ...
  switch l.type
    ...
    %% new layers added by ZengHui at 20170707
    case 'bilinearpool'
%       res(i+1).x = vl_nnbilinearpool(res(i).x);
      res(i+1).x = vl_nnbilinearpool_adaptiveMultiCrop(res(i).x);
      
    case 'G2DeNet'
      res(i+1) = nnfspool_forward(ropts, res(i), res(i+1)) ;
      res(i+1).cpu = 1;  
    ...
    case 'softmax_regressionloss'
      res(i+1).x = vl_softmax_regressionloss(res(i).x, l.class,[],net.meta.lossType) ;
      
    case 'multiCrop_agg'
      res(i+1).x = multiCrop_agg(res(i).x,l);
      
    case 'subject_pool'
      res(i+1).x = subject_pool(res(i).x,l);
...
%         backprop = vl_nnbilinearpool(res(i).x, res(i+1).dzdx);
        backprop = vl_nnbilinearpool_adaptiveMultiCrop(res(i).x, res(i+1).dzdx);
        res(i).dzdx = updateGradient(res(i).dzdx, backprop);
      
      case 'G2DeNet'
        backprop = nnfspool_backward(ropts, res(i), res(i+1)) ;    
        res(i).dzdx = updateGradient(res(i).dzdx, backprop);
      ...
      case 'softmax_regressionloss'
        res(i).dzdx = vl_softmax_regressionloss(res(i).x, l.class, res(i+1).dzdx, net.meta.lossType) ;
        
      case 'multiCrop_agg'
        res(i).dzdx = multiCrop_agg(res(i).x,l,res(i+1).dzdx);
      
      case 'subject_pool'
        res(i).dzdx = subject_pool(res(i).x,l,res(i+1).dzdx);
