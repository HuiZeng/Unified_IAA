...
for t=1:params.batchSize:numel(subset)
  ...
  for s=1:params.numSubBatches
    ...
    if strcmp(params.imdb.database,'AVA') && size(inputs{4},1)==1
    ...
    if size(net.vars(p).value,3) > 1
        ...
        switch params.imdb.meta.lossType
            case 'CE'
                predictions(batch) = squeeze(gather(vl_nnsigmoid(net.vars(p).value))); 
            case {'MSE','Huber'}
                predictions(batch) = squeeze(gather(net.vars(p).value)); 
        end
